...
/**
 * Output transformation that applies text styles and peer cursor/selection highlighting
 */
class CollaborativeOutputTransformation : OutputTransformation {
    ...
    override fun TextFieldBuffer.transformOutput() {
        // Apply text style operations (bold, italic, underline, strikethrough)
...
/**
 * A collaborative text editor with real-time peer cursor display and text styling.
 */
@OptIn(ExperimentalFoundationApi::class)
@Composable
fun CollaborativeEditor(
    textFieldState: TextFieldState,
    selectionPeers: Map<ActorID, Selection?>,
    styleOperations: List<OperationInfo.StyleOpInfo> = emptyList(),
    ...
    val outputTransformation = remember {
        CollaborativeOutputTransformation()
    }

    LaunchedEffect(selectionPeers, styleOperations) {
...
private fun RemoteCursorOverlay(
    ...
    Box(modifier = modifier) {
        selectionPeers.filterValues { it != null }.forEach { (actorId, selection) ->
            key(actorId) {
                selection!!
...
/**
 * Calculate cursor position using text layout result
 */
@Composable
